...
internal class UIvsGameInputHandlerEditor : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        using (new EditorGUI.DisabledScope(true))
        {
            ...
            {
                var state = ((InputHandler)target).m_State;
                EditorGUILayout.LabelField("State", state.ToString());
                var style = ((InputHandler)target).m_ControlStyle;
                EditorGUILayout.LabelField("Controls", style.ToString());
                if (style == InputHandler.ControlStyle.GamepadJoystick)
                {
                    var uiEngaged = ((InputHandler)target).m_UIEngaged;
